"""
02_erd_ers：建 ERD/ERS pipeline 跑通。
...
pipeline（跟 01 ERP 的区别在末端：不是 ERP 时域叠加，而是 TFR 时频 + 基线归一化看 ERD/ERS）：
  LoadData → Bandpass(1-40 IIR) → Notch(50Hz) → Epoch(按 MI 类切分) → TFR(Morlet, ERD/ERS)
不做 Re-reference（运动想象 ERD/ERS 对参考不敏感，少一个待回填的通道列表，先求跑通）。
"""
    ...
    return {
        "graph": {
            ...
            "links": [
                ...
                {"id": "l3", "from": {"node": "nf", "port": "output"}, "to": {"node": "ep",  "port": "input"}},
                {"id": "l4", "from": {"node": "ep", "port": "output"}, "to": {"node": "tfr", "port": "input"}},
            ],
        },
    }
