...
├── init.sql       # 一键入口：schema + roles/permissions（不含 demo 用户）
└── init_core.sql  # 生产/准生产入口：schema + roles/permissions
```
...
部署脚本 `deploy/deploy.sh` 已自动调用 `init.sql`，无需手动执行。demo 用户请在本地私有 seed 或部署环境中创建，不要把默认密码或 hash 写入公开仓库。
