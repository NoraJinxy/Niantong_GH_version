...
flowchart TB
  ...
  Pipeline --> Store[DerivedDatasetStore]
...
- `pipeline/` 可以读写 DB，但应通过清晰的执行上下文和 DerivedDatasetStore 管理 DerivedDataset 输出。
...
Model 层当前集中在 `models/study.py`，包含 Study、Recording、Pipeline、PipelineExecution、PipelineJob 等对象；`DerivedDataset` 已独立到 `models/derived_dataset.py`。注意 `Dataset` / `DatasetUpload` ORM 类已在「A 重构」中删除并改名为 `Recording` / `RecordingVersion`，`models/study.py` 中不再存在这两个类。后续可继续按业务对象拆分：
