...
| Pipeline | `pipeline/` | 工作流注册、校验、执行、缓存、StudyOutput 写入 |
| Engine | `engine/` | EEG 算法和 MNE 相关计算 |
| Task | `tasks/` | Celery app 和后台任务入口 |
| Utils | `utils/` | JWT、密码 hash 等纯工具 |

后端分层的关键约束：Router 负责 API 入口和权限，Service 负责锁、任务事件、依赖保护等跨模块规则，Pipeline 层负责输入解析、节点执行和 StudyOutput 登记，Task 层只作为后台调度信封，不应成为 Execution 事实源。
...
flowchart TB
  ...
  Pipeline --> Store[StudyOutputStore]
...
- `pipeline/` 可以读写 DB，但应通过清晰的执行上下文和 StudyOutputStore 管理 StudyOutput 输出。
...
| `schemas/study_output.py` | StudyOutput 元信息、预览、cleanup |
...
Model 层当前集中在 `models/study.py`，包含 Study、Recording、Pipeline、PipelineExecution、PipelineJob 等对象；`StudyOutput` 已独立到 `models/study_output.py`。注意 `Dataset` / `DatasetUpload` ORM 类已在「A 重构」中删除并改名为 `Recording` / `RecordingVersion`，`models/study.py` 中不再存在这两个类。后续可继续按业务对象拆分：

| 当前集中模型 | 建议拆分 |
|---|---|
| `Study` / `StudyMember` | `study.py` |
| `DatasetAsset` / `DatasetVersion` / `StudyDatasetMount` / `Subject` / `DatasetFile` / `Recording` / `RecordingVersion` / `DatasetFileDerivation` / `DatasetVersionReference` / `DatasetWithdrawalRequest` | `dataset.py` |
| `PipelineDefinition` / `PipelineExecution` / `PipelineJob` | `pipeline.py` |
| `StudyOutput` | 已独立在 `study_output.py` |
...
| StudyOutput 发布 | 临时文件写完、hash 成功、DB 记录成功后再视为正式产物 |
