...
定位
: PipelineExecutor、Dispatcher、ArtifactStore、Cache、Celery
...
| ArtifactStore | `pipeline/artifacts.py` | 写入输出文件并登记 `derived_datasets` |
...
ArtifactStore 将节点输出写到 content-addressed 目录：
...
| 字段 | 含义 |
|---|---|
| `storage_uri` | content-addressed 目录的逻辑 URI |
| `sha256` | 内容哈希，缓存命中和复用的依据 |
| `retention_status` | 保留/隐藏/待清理状态 |
| `execution_id` / `job_id` | 来源 Execution 和节点 Job |
