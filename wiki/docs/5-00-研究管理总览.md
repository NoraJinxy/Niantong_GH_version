...
数据库
: `studies` · `study_members` · `study_settings` · `study_dataset_mounts` · `pipeline_definitions` · `pipeline_executions` · `pipeline_execution_inputs` · `study_outputs`
...
| **Execution** | 执行项 | 这一次实际怎么跑 | 实际输入快照、Pipeline 定义快照、运行参数、任务状态、日志、StudyOutput、依赖 | “当前数据”的动态引用、可被后续编辑改变的 Pipeline 定义 |

Dataset 是研究输入资产，StudyOutput 是 Execution 输出资产。Study 通过挂载引用 Dataset；Execution 通过冻结快照引用具体文件。
...
- StudyOutput `produced_by_execution_id` 指向 Execution；`upstream_dataset_ids` / `upstream_recording_ids` 记录派生来源。
...
原始 EEG 文件
  ...
  → 节点产物写入 study_outputs（含 storage_uri / sha256 / 上游链）
  → StudyOutput 可作为下游 Execution 的输入（链式分析）
```
...
- StudyOutput 可清理大文件，但不能删除 Execution 事实链。
- 被下游 Execution 依赖的 StudyOutput 受保护，无法直接物理清理。
...
| StudyOutput 写入和保留状态 | <span class="elys-badge elys-badge--done">已实现</span> | `study_outputs.retention_status` |
...
| Execution 依赖自动写入 | <span class="elys-badge elys-badge--done">已实现</span> | 节点输入解析识别上游 StudyOutput 并写 `pipeline_execution_dependencies` |
| StudyOutput 清理任务 | <span class="elys-badge elys-badge--done">已实现基础版</span> | `study_output_cleanup` 逻辑清理 `temporary/cached`，被依赖则跳过 |
...
| [5-30 Execution 管理](5-30-Execution管理.md) | Execution 生命周期、输入快照、异步任务、StudyOutput、manifest、依赖和清理 | 改运行、任务队列、输出追溯时 |
...
7. Dataset 异步导入已落地（`/recordings/import-async` 请求内落盘 + Celery worker 经 `materialize_recording_import` 转 FIF）；后续推进更彻底的 staged upload（大文件分片直传）。
