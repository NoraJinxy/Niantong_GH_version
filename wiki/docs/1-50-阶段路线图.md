...
| 目标 | 状态 | 关键页 |
|---|---|---|
| 导入级 QC（Montage / 事件 / 信号质量） | 规划 | [M2](8-00-功能模块总览.md) |
| `tasks` 表 + Celery worker | 规划 | [2-60](2-60-任务队列与异步架构.md) |
| 节点级缓存（content_hash） | 规划 | [M3-60](7-40-工作流执行与后台任务.md) |
| 派生产物（`derivatives/preprocessing/`） + `derived_datasets` 写入 | 部分接入 | [3-45 DerivedDataset](3-45-DerivedDataset.md) |
| 进度推送（SSE / WebSocket） | 规划 | [M3-50](2-60-任务队列与异步架构.md) |
...
| 项 | 内容 |
|---|---|
| 资源档位 | PI / PI Plus / PI Pro 上线（见 [M0-40](8-00-功能模块总览.md)） |
