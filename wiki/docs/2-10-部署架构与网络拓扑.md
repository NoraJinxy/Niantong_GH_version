...
入口服务器（`8.135.40.150`）跑 Nginx + Vue dist；计算服务器（`8.135.57.42`）跑 FastAPI + PostgreSQL + Redis + 研究项数据目录。正式域名为 `elysbrain.site`，备案号为 `粤ICP备2026062729号`；主站使用 `elysbrain.site`，数据/API 子域使用 `data.elysbrain.site`，URL 切换不影响数据层。
...
| 服务器 | 域名 / IP | 职责 | 对外开放 |
|---|---|---|---|
| **入口服务器** | 当前 `http://8.135.40.150`；正式主站 `elysbrain.site` / `www.elysbrain.site` | Nginx · Vue dist · 轻 API 反代 | 80 / 443 |
| **计算服务器** | 当前 `http://8.135.57.42`；正式数据域名 `data.elysbrain.site` | Nginx · FastAPI · PostgreSQL · Redis · 研究项数据目录；Celery 预留 | 80 / 443 |
...
| 主机记录 | 类型 | 记录值 | 用途 | 优先级 |
|---|---|---|---|---|
| `@` | `A` | `8.135.40.150` | 主站，访问 `elysbrain.site` | 必需 |
| `www` | `CNAME` | `elysbrain.site` | 访问 `www.elysbrain.site`，便于用户习惯输入 | 建议 |
| `data` | `A` | `8.135.57.42` | 上传、下载、波形、文件索引、大数据 API | 必需 |
...
| 访问地址 | 用途 | 流量类型 |
|---|---|---|
| `http://8.135.40.150` | 当前主站 · 前端 · 登录 · 轻 API | 小流量 |
| `http://8.135.57.42` | 当前文件上传 / 下载 · 波形 · 时频 · 大图 | 大流量 |
