...
| `Pipeline` | 工作流 | 流程身份、版本、模板标记（「当前认可结果」无专用指针列，由 `derived_datasets.retention_status='current'` 反查，见 [3-30](3-30-Study与Pipeline表.md)） |
| `Execution` | 执行项 | 一次执行记录、输入快照、节点记录、输出索引 |
| `DerivedDataset` | 派生数据集 | clean raw、epochs、ERP、图表、报告、缓存等产物索引 |
...
flowchart LR
  ...
  DerivedDataset[DerivedDataset 派生数据集]
...
| DerivedDataset | `derived_datasets` · `pipeline_execution_dependencies` | 派生数据集统一登记 + 依赖追溯 |
