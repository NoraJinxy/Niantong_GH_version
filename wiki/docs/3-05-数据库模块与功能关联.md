...
flowchart TB
  ...
  subgraph M5["05_derived · 派生数据与 Execution I/O"]
...
**为什么 05_derived 单独成模块**：`derived_datasets`、`pipeline_execution_inputs`、`pipeline_execution_dependencies` 都同时依赖 `pipeline_executions`（在 04）和 `dataset_files` / `derived_datasets` 自身（循环引用要在 derived_datasets 之后才能建表），所以它们必须在 04 之后加载。把它们聚合成一个"派生与 I/O"模块。
...
**所有"原始数据"层的事实源**——EEG 文件的资产、版本、subject、采集记录（recording）、记录的上传版本、文件物理位置。不含 Pipeline 产出的派生数据（那是 05）。
...
**Pipeline 的"定义"和"执行骨架"**——保存流程结构（节点+连线+参数）、执行的元信息和状态、每个节点运行的状态/输入输出哈希。不含运行的实际输入快照（那在 05 的 `pipeline_execution_inputs`）和运行的输出产物（那在 05 的 `derived_datasets`）。
...
| `pipeline_executions` | `id` UUID | 一次执行，含 status / definition_snapshot / manifest_json / execution_mode / save_policy |
...
| `execution_mode` | trial / analysis / replay / system | 运行类型 |
| `save_policy` | temporary / current / pinned / discard | 决定输出 `derived_datasets.retention_status` 的初值 |
...
3. INSERT INTO pipeline_executions (status='running', definition_snapshot=..., manifest_json=..., execution_mode='analysis')
4. INSERT INTO pipeline_execution_inputs (...) × N  -- 冻结实际输入（见 05）
5. 按拓扑序遍历节点：
   ├─ INSERT INTO pipeline_jobs (status='running', topo_index=k, node_hash=...)
   ├─ 执行节点 → 写入 derived_datasets（见 05）
...
## 6. 模块 5：`05_derived.sql` 派生数据与 Execution I/O
...
| 表 | 主键 | 说明 |
|---|---|---|
| `derived_datasets` | `id` UUID | **派生数据集统一表**——Pipeline 各节点产出的文件登记于此 |
| `dataset_file_derivations` | `id` UUID | 文件级派生关系（source_file → derived_file，可关联 execution 和 derived_dataset）|
...
| 数据语义 | `data_type` · `subject_id` · `bids_subject_id` · `session` · `task` · `run_label` · `condition` | 这条派生数据的"业务身份" |
...
```
节点执行完成 → 输出文件按内容寻址写到 studies/{study_id}/derived/{sha256[:2]}/{sha256}/...
  ↓
INSERT INTO derived_datasets (
    ...
    storage_uri='elys://studies/{study_id}/derived/...',
    sha256, file_size,
...
INSERT INTO dataset_file_derivations (source_file_id=输入文件, derived_file_id=新文件, execution_id, derived_dataset_id)
```

### 数据流：跨 Execution 依赖

```
Execution B 的 LoadData 引用了 Execution A 产出的 derived_dataset
  ↓
...
INSERT INTO pipeline_execution_dependencies (execution_id=B, depends_on_execution_id=A, upstream_dataset_id=A_derived_id, dependency_kind='upstream_execution')
  ↓
这一行保护 A 的派生数据不被 cleanup（ON DELETE RESTRICT）
```

### 谁会读它

- 派生数据浏览页（按 `produced_by_execution_id` / `subject_id` / `data_type` / `tags` 筛选）
...
- `derived_dataset_cleanup` — 清理过期 derived_datasets
...
### 场景 A：列出某用户能看到的所有派生数据

```sql
SELECT dd.*
FROM derived_datasets dd
...
| `derived_datasets` | `DerivedDataset` | `models/derived_dataset.py` |
...
- [3-45 DerivedDataset](3-45-DerivedDataset.md) — derived_datasets 深入
