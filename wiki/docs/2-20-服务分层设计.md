...
| **Pipeline** | `backend/app/pipeline/` | 节点规格注册表、LoadData 解析、执行器、Dispatcher、StudyOutputStore、缓存、manifest |
...
├── pipeline/            registry · validator · load_data · executor · dispatcher · study_output_store · cache · execution_manifest · background · save_settings · timeseries 等；nodes/*.json 8 个 NodeSpec
...
│   └── pipelines.ts         NodeSpec / 工作流 CRUD / 校验 / 运行 / LoadData / outputs
