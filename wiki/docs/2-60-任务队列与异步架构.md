...
| Pipeline Execution 异步派发 | <span class="elys-badge elys-badge--wip">部分接入</span>，`/pipelines/{id}/executions` 创建 `pipeline_executions(status=queued)` 后派发 Celery；但仅当 `PIPELINE_EXECUTION_MODE=celery` 或 `auto` 且 ping 到 worker 时才派发，否则在 API 进程内联同步执行（见下方 inline 说明 + [7-40](7-40-工作流执行与后台任务.md)） |
...
| StudyOutput 写入 | <span class="elys-badge elys-badge--done">已实现基础版</span>，新写入走 Study content-addressed Store |
| Dataset 导入 | <span class="elys-badge elys-badge--done">同步 + 异步均可用</span>，`/recordings/import-async` 请求内落盘后派 Celery，worker 经 `materialize_recording_import` 转 FIF；`/recordings/import` 同步入口保留 |
...
| 文件管理任务 | <span class="elys-badge elys-badge--wip">部分接入</span>，`study_output.cleanup` 有实际清理逻辑，导入/构建/重建为任务骨架 |
...
| `StudyOutput / 输出产物` | Execution 或 Job 产生的输出（取代旧 Artifact 概念） | 按保留策略管理 |
...
| `study_output.preview` | StudyOutput | `preview` | 生成波形、表格、图像预览 |
| `study_output.cleanup` | StudyOutput | `maintenance` | 清理 temporary/cached 输出 |
| `export.package` | Dataset / Study / Execution | `export` | 导出数据、报告、Execution Manifest |

MVP 已实现 `pipeline_execution` 和文件管理统一任务入口；`study_output.cleanup` 已可用，`dataset.import/raw_bids_build/canonical_fif_rebuild` 目前主要用于登记任务和事件，真实导入替换仍需 staged upload。
...
| `resource_kind` / `resource_id` | 关联 Execution、Dataset、StudyOutput 等 |
...
`async_tasks.status` 的 CHECK 约束只有 `queued / running / succeeded / failed / canceled / retrying` 六个值（无 `dispatched`、`canceling`、`waiting_user_input`——`dispatched` 是 `task_events.event_type` 事件名，`waiting_user_input` 是 Execution 状态而非 Task 状态）：
...
sequenceDiagram
  ...
  loop 每个 Job
    W->>DB: job -> running
    W->>FS: 读取 Dataset File 或上游 StudyOutput
    W->>FS: 写临时输出
    W->>DB: 登记 StudyOutput / 进度事件
...
- StudyOutput 正式发布前先写临时目录，成功后登记数据库。

> **inline 同步执行路径（当前真实主力之一）**：上图是带 worker 的异步链路。代码在派发前先调 `should_execute_pipeline_inline(settings)`，当 `PIPELINE_EXECUTION_MODE=inline`，或 `auto` 模式 ping 不到 worker 时，改为 `execute_pipeline_execution_inline` 在 API 进程内同步跑完——此时"API 线程只做创建和派发、不执行重计算"这条不成立。阿里云生产部署默认 `celery`（走异步），本地 `auto` 无 worker 时走 inline。详见 [7-40 §2](7-40-工作流执行与后台任务.md)。
...
同步导入 `/recordings/import` 仍保留；异步导入已落地：`POST /recordings/import-async`（旧入口 `/import-task`）在请求内把 `UploadFile` 落盘归档，再派 Celery，worker `run_dataset_import` 经 `materialize_recording_import` 完成校验+FIF 转换+写库，进度走 `task_events`。更彻底的 staged upload（大文件分片直传、不在请求内落盘）仍待后续。
...
| StudyOutput 清理 | 清理前必须检查下游依赖和 pinned/current 状态 |
...
取消不能删除已经产生的 Execution 记录；已经登记的 StudyOutput 按保留策略处理。
...
```text
job_hash = hash(node_type + node_version + params + input_file_hash + upstream_study_output_hash + software_version)
```

缓存命中时：

- `pipeline_jobs.status='cached'`。
- 新 Execution 可引用已有 StudyOutput。
- 不重复写大文件。
...
| File task route | `dataset.import` / `raw_bids_build` / `canonical_fif_rebuild` / `study_output.cleanup` | 已统一进入 `run_file_task` |
...
4. 已新增 StudyOutput `retention_status`，让清理有状态。
5. 已新增 Task 列表、详情和事件查询 API。
6. 已新增文件管理任务入口和 `study_output.cleanup` 逻辑清理。
...
- 不让前端直接读写文件路径。
- 不在清理任务里物理删除 current/pinned/downstream-dependent StudyOutput。
