...
当前后端以 FastAPI 为入口，按 `routers / schemas / services / models / pipeline / engine / tasks / utils` 组织；路由仍以 `/studies` 兼容当前代码语义，产品文档统一解释为 Study。文件访问、Task 查询/取消/重试/SSE、Execution Manifest、Execution lineage、DerivedDataset cleanup 等接口已经补齐，实际路由数量以代码为准，不再把固定数量作为架构约束。
...
| ORM 模型 | `models/*.py` |
| 请求响应契约 | `schemas/*.py` |
| 数据导入与 FIF 转换 | `routers/datasets.py` |
...
4. **Dataset 导入异步化**：从同步 HTTP 请求迁移为 staged upload + 后台任务。
