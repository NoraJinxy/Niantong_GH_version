...
| 端点总数 | 80+（孤儿/别名清理后仍在持续扩展，**以代码为准**，不把固定数量当约束；新增如 execution 维度 `/outputs`、跨 Study `include_cross_study` 等已超早期计数） |
...
发版 / 申请撤回 / admin 审核 / admin 紧急下架（superadmin 不参与数据集生命周期，见 [3-25 §6.4](3-25-数据集生命周期与发布机制.md)）。5 个端点。

**已识别问题**：`emergency-takedown` 在 `/dataset-versions/{id}` 下，但实际是 admin 治理操作，权限边界与 owner 操作混在一起。建议挪到 `/dataset-withdrawals/emergency-takedown/{version_id}`。
