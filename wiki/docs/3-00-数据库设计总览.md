...
| 发布 ≠ 分享 | Dataset 默认 `status=working` / `visibility=private`（两者独立：`status` 是内部生命周期、`visibility` 是可见范围）。可见范围（私有/共享/公开）作为数据集 UI 状态徽章展示，发布默认私有、由负责人在发布后**只升不降**地显式开放（无降级接口）；发布/撤回的完整治理流程仍在完善 |
...
| `schema/03_datasets.sql` | 数据集与文件 | `dataset_assets` · `dataset_versions` · `study_dataset_mounts` · `subjects` · `recordings` · `recording_versions` · `dataset_files` · `dataset_montages`（自定义电极位置文件，供「通道定位」节点选用）|
| `schema/04_pipelines.sql` | Pipeline 核心 | `pipeline_definitions` · `pipeline_executions` · `pipeline_jobs` |
| `schema/05_outputs.sql` | 结果与 Execution I/O | `study_outputs` · `execution_outputs` · `dataset_file_derivations` · `pipeline_execution_inputs` · `pipeline_execution_dependencies` |
...
erDiagram
  ...
  pipeline_executions ||--o{ study_outputs : produces
  pipeline_executions ||--o{ execution_outputs : produces_or_reuses
  study_outputs ||--o{ execution_outputs : attributed_by
  pipeline_executions ||--o{ pipeline_execution_dependencies : depends_on
  study_outputs ||--o{ pipeline_execution_dependencies : protects
  study_outputs ||--o{ dataset_file_derivations : registered_as
```
...
| [3-45 StudyOutput](3-45-StudyOutput.md) | `study_outputs` 详细字段、生命周期、retention 策略 | 改结果登记、清理策略时 |
...
| 文件索引 | `dataset_files` · `dataset_file_derivations` | `03_datasets.sql` · `05_outputs.sql` | [3-20](3-20-Dataset与采集记录表.md) · [4-70](4-70-文件角色URI与追溯关系.md) |
...
| 执行项 | `pipeline_executions` · `pipeline_jobs` · `pipeline_execution_inputs` · `pipeline_execution_dependencies` | `04_pipelines.sql` · `05_outputs.sql` | [3-40](3-40-Execution与Artifact追溯表.md) |
| 输出产物 | `study_outputs` · `execution_outputs`（执行↔输出多对多：每次执行产出/复用记一条边，运行面板/执行详情据此统计「本次产物」，绕开 content-addressed 去重令 `produced_by_*` 永指首产者的问题） | `05_outputs.sql` | [3-45](3-45-StudyOutput.md) |
...
| Execution 依赖 | 已落表 `pipeline_execution_dependencies`，上游结果输入会自动写依赖 |
| 结果登记 | 已落表 `study_outputs` |
...
2. 为结果增加 pin/unpin（钉住 / 取消钉住，标记某结果不被自动清理）的产品级操作；cleanup 已有基础逻辑。
