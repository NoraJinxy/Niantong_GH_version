...
| 发布不是 MVP 核心 | Dataset 默认 `status=working` / `visibility=private`（两者独立：`status` 是内部生命周期、`visibility` 是可见范围）。可见范围（私有/共享/公开）已作为数据集 UI 状态徽章展示且 owner 可手动调整；发布/撤回的完整治理流程仍在完善 |
